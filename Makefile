...
# This is the "keep quality left" idea from harness engineering: shift the
# sensors left so an agent (or a human) can self-correct before code leaves
# the machine.
#
# INSTANTIATING THIS TEMPLATE: replace every {{SERVICE}}, {{SERVICE_REPO}}
# and {{RICHEST_AGGREGATE}} placeholder -- `scripts/new-service.sh` does
# this substitution for you. See HARNESS.md.
...
# Thresholds/workers/timeout live in .gremlins.yaml so both use identical
...
MUTATION_FAST_PKG  := ./internal/domain/{{RICHEST_AGGREGATE}}
...
help:
	@echo "{{SERVICE}} — local quality gate (targets mirror .github/workflows/ci.yml)"
	@echo ""
